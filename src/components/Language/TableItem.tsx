...
import { delLanguage, putLanguage } from '../../api-calls/language'
...
function TableItem({ urlProgLang, id, handleRefresh }: TableItemProps) {
  ...
  const handleDelete = async () => {
    ...
    const res = await delLanguage(id)
    if (!res.error) {
      handleToast(idToast, res)
      setEdit(false)
      handleRefresh?.()
  ...
  return (
    <tr className='bg-white border-b '>
      {edit ? (
        ...
        <td className='px-6 py-4 font-medium text-gray-900 whitespace-nowrap truncate max-w-2xl'>
          {urlProgLang}
        </td>
      )}
